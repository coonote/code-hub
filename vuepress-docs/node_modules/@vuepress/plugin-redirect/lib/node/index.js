...
<head>
  ...
  <script>
    ...
    const routePath = pathname.slice(${i.length});
...
`,T=async({dir:a,options:e,pages:c},n)=>{const t=c.filter(({pathLocale:l})=>l==="/").map(({path:l})=>l),i={};c.filter(({pathLocale:l})=>l!=="/").forEach(({path:l,pathLocale:o})=>{const r=l.replace(o,"/").replace(/\/$/,"/index.html");t.includes(r)||(i[r]??=[]).push(o)});const{succeed:$}=d.load("Generating locale redirect files");await Promise.all(m(i).map(async([l,o])=>{const r=a.dest(p(l));s.existsSync(r)||(await s.ensureDir(f.dirname(r)),await s.writeFile(r,M(n,o,e.base)))})),$()},B=a=>`<!doctype html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="robots" content="noindex">
